        ...
        if self.in_proximity_accuracy().count() > 0:
            for agent, distance in self.in_proximity_accuracy():
                if isinstance(agent, Rabbit) and distance >7 and distance< self.chasing_radius:
                    ...
                    self.move = 0.3*Vector2(loc_rabbit_x-x1,loc_rabbit_y-y1)
                    self.pos += self.move
...
df=(
    Simulation(Config(radius=40, movement_speed=1.5,seed=1))
    .batch_spawn_agents(
        20,
...
plot.savefig('plot_chaser_without_age_12.png', dpi=300)